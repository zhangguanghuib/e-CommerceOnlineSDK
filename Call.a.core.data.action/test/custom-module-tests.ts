...
fixture('Custom Module tests').page('http://localhost:4000/modules?type=product-feature&theme=spring');
...
test('validate image is rendering on the page', async (testController: TestController) => {
     ...
     await testController
             .expect(imageTag.exists) 
             .eql(true, 'Image not rendered on the page as expected');
});
...
test('validate product heading is rendering on the page', async (testController: TestController) => {
     ...
     await testController
            ...
            .eql(true, 'Product heading not rendered on the page as expected');
     await testController
            .expect(productTitle.innerText)
            .eql('Retro Horn Rimmed Keyhole Nose Bridge Round Sunglasses', 'Product title is not rendering on the page');
});

fixture `DAPI preview API Test`.before(async (ctx) => {
    // @ts-ignore
    process.env['NODE_TLS_REJECT_UNAUTHORIZED'] = '0';
    const modulePreviewRes = await fetch('https://localhost:4000/_sdk/dapi/preview?module=product-feature');
