...
export const getProductsByCategoryList = async (input: ProductListInput, context: IActionContext): Promise<ProductSearchResult[]> => {
    console.log('ProductsByCategoryListAction - from customizatin by GHZ');
    ...
    return searchByCriteriaAsync(
        ...
        .then((products: ProductSearchResult[]): SimpleProduct[] | ProductSearchResult[] => {
        ...
        .catch(error => {
            context.telemetry.error(`Error in ProductByCategory action: ${error}}`);
