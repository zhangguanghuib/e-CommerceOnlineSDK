...
export class ProductListInput implements IActionInput {
    ...
    constructor(listMetadata: IProductListMetaData, actionContext: IActionContext) {
        ...
        if (!Object.keys(ProductListType).includes(listMetadata.listType)) {
            throw new Error(`Invalid listType: ${listMetadata.listType}`);
        }

        const productListTypeEnumValue = ProductListType[listMetadata.listType as keyof typeof ProductListType];

        // switch (ProductListType[listMetadata.listType]) {
        switch (productListTypeEnumValue) {
